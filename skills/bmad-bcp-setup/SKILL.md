...
1. Leia `assets/module.yaml` para metadados e definições de variáveis (o campo `code` é o identificador do módulo).
2. Verifique se `{project-root}/_bmad/config.yaml` existe — se já houver seção `bcp`, informe que é uma atualização.
3. Verifique config legacy per-module em `{project-root}/_bmad/bcp/config.yaml` e `{project-root}/_bmad/core/config.yaml`. Se existirem:
