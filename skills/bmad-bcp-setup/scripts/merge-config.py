...
- writes the module variable values (``bcp_*``) into
  ``_bmad/custom/config.toml`` under ``[modules.<code>]`` — the layer
  ``resolve_config.py`` reads with higher priority than the installer
  defaults in ``config.toml`` (issue #36, toml-first). Existing comments and
  other sections in that human-owned file are preserved (tomlkit round-trip);
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Merge module config into shared _bmad/config.yaml (core keys) "
        "and _bmad/custom/config.toml (module section) with anti-zombie pattern."
    )
    parser.add_argument(
        "--config-path",
        required=True,
...
def merge_config(
    ...
    """Merge answers into config.yaml, applying anti-zombie pattern.

    Writes only core keys at root. The module section is NOT written here — it
    goes to custom/config.toml (see write_module_toml). Any stale legacy module
    section in config.yaml is stripped (yaml→toml migration).
    ...
    # Toml-first (issue #36): the module section is written to
...
def coerce_toml_scalar(value):
    ...
    Every ``bcp_*`` value is a string by design (module.yaml uses
...
def main():
    ...
    # Write the module section to custom/config.toml (toml-first, issue #36).
