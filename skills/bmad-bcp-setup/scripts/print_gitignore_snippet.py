...
SNIPPET = """\
# BCP customizations — keep team .toml overrides versioned, .user.toml private
!_bmad/custom/
_bmad/custom/*
!_bmad/custom/*.toml
...
def is_allowlisted(gitignore: Path) -> bool:
    """Return True if .gitignore has an active allowlist entry for
    `_bmad/custom/`. Commented lines are skipped so a commented-out rule
    does not mask a missing real one.
    """
...
def main() -> int:
    ...
    if not gitignore.exists():
        ...
        sys.stdout.write(
            f"Append the snippet below to {gitignore} to version BCP "
            f"customize overrides:\n\n"
        )
