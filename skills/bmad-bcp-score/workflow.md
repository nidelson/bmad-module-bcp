...
7. **Config (toml-first):** rode `python3 {project-root}/.claude/skills/bmad-bcp-score/scripts/bcp_config.py --project-root {project-root}` e leia o objeto `bcp` do JSON. O helper resolve **toml-first**: `[modules.bcp]` do `config.toml` (via `resolve_config.py` do core — honra os overrides de `custom/config.toml`), com **fallback** por chave para a seção `bcp` legada do `config.yaml`, e o **default** do `module.yaml` como último recurso. Chaves que importam aqui: `bcp_confidence_threshold` (default 0.75), `bcp_non_interactive_default` (default yes), `bcp_overwrite_estimated_hours` (consentimento — se `no`, não sobrescreva `estimated_hours`: apenas anexe o bloco `bcp.*` e informe), `bcp_reference_h_per_bcp` (reference rate frozen para a âncora de alavancagem — se ausente, o script usa o seed; **nunca** o fator recalibrado). Sem config → o helper já devolve os defaults; siga (módulo pode não ter rodado setup).
