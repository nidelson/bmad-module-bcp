...
from .conftest import SKILLS, run_script

REGISTER = SKILLS / "bmad-bcp-setup/scripts/register-party-agent.py"
FRAGMENT = SKILLS / "bmad-bcp-setup/assets/agent-manifest-fragment.csv"

AGENT_KEY = "bmad-bcp-agent-bruno"
...
CUSTOM_DESCRIPTION = "Coach do squad. Bordão: 'Régua antes de régua'."
CUSTOM_TITLE = "Coach de Complexidade (SIP)"

LEADING_COMMENT = "# Bruno (BCP) no roster do party-mode — módulo custom não entra no [agents] base."
TRAILING_SECTION_COMMENT = "# BCP: valores custom pinados (fonte única)."
...
def _run(root: Path, *extra: str):
    result = run_script(REGISTER, "--project-root", str(root),
...
def test_creates_entry_on_fresh_install(tmp_path: Path):
    ...
    assert entry["name"] == "Bruno"
    assert entry["module"] == "bcp"
