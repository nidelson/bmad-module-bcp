...
installed at .claude/skills/ — only the config files at _bmad/ root need
to persist.
...
Exit codes: 0=success (including nothing to remove), 1=validation error,
2=runtime error
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--module-code",
        required=True,
        help="Module code being cleaned up, e.g. 'bcp'",
    )
    parser.add_argument(
        "--also-remove",
        action="append",
    ...
    parser.add_argument(
        "--skills-dir",
        help="Path to .claude/skills/ — enables safety verification that "
        "skills are installed before removing legacy copies",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def find_skill_dirs(base_path: str) -> list:
    """Return the leaf directory name of every directory containing SKILL.md."""
    skills = []
    root = Path(base_path)
    if not root.exists():
        return skills
...
def verify_skills_installed(
    ...
    """Verify skills in legacy directories exist at the installed location.
...
def cleanup_directories(
    ...
    for dirname in dirs_to_remove:
        ...
        if verbose:
            print(f"Removing {target} ({file_count} files)", file=sys.stderr)
...
def main():
    ...
    # Directories to remove: this module + core + extras, order-preserving dedup
    ...
    if args.skills_dir:
        if args.verbose:
            print(f"Verifying skills installed at {args.skills_dir}", file=sys.stderr)
