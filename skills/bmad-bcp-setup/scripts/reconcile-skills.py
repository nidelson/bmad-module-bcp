...
# dependencies = []
# ///
"""Self-heal BCP skills deployed in a consumer project.
...
`--source`). It force-syncs every BCP skill directory and prunes known
...
with `skipped_no_source` so the caller (bmad-bcp-setup) can continue.
...
# Folders shipped by a previous BCP version that were renamed/removed and
...
LEGACY_SKILL_FOLDERS = {
    # BCP nunca renomeou/removeu uma skill — nenhum folder legacy a podar.
    # (Mantido o mecanismo para paridade com o BCP: mapeia legacy -> canonical.)
}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Reconcile deployed BCP skills against an authoritative source."
    )
    parser.add_argument(
        "--project-root",
        required=True,
    ...
    parser.add_argument(
        "--source",
        help="Path to a fresh BCP source tree (a directory containing "
...
def discover_source(project_root: Path, cache_root: Path, verbose: bool):
    """Locate the BMAD custom-module cache for BCP from the manifest.

    Parses {project-root}/_bmad/_config/manifest.yaml without a YAML
    dependency: finds the `- name: bcp` module item and reads its
    ...
    if not repo_url or repo_url == "null":
        if verbose:
            print("No bcp repoUrl in manifest", file=sys.stderr)
        return None
...
def main():
    ...
    deployed_version = read_module_version(
        skills_dir / "bmad-bcp-setup" / "assets" / "module.yaml"
    )
            ...
            json.dumps(
                {
                    ...
                    "notice": (
                        "No BCP source/cache located — nothing to "
    ...
    source_version = read_module_version(
        source_skills_dir / "bmad-bcp-setup" / "assets" / "module.yaml"
    )
    ...
    if converged and deployed_version != source_version:
        result["notice"] = (
            f"BCP reconciled {deployed_version} -> {source_version}. "
            f"bmad-bcp-setup may have been updated in place; re-run "
            f"/bmad-bcp-setup once more to execute the current version."
        )
